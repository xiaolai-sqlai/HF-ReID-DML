...
if __name__ == '__main__':
    ...
    data_dir = osp.join(opt.dataset_root, "cuhk03-np/detected")
    os.environ["CUDA_VISIBLE_DEVICES"] = opt.gpus

    test_set = [(osp.join(data_dir, 'bounding_box_test',line), int(line.split('_')[0])) for line in os.listdir(osp.join(data_dir, 'bounding_box_test')) if ("jpg" in line or "png" in line) and "-1" not in line]
    query_set = [(osp.join(data_dir, 'query',line), int(line.split('_')[0])) for line in os.listdir(osp.join(data_dir, 'query')) if ("jpg" in line or "png" in line)]
    ...
    net = HFNet(num_classes=767, net=opt.net, decoder=opt.decoder)
